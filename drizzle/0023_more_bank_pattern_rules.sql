-- ---------------------------------------------------------------------------
-- Hand-written: add more Spanish bank-statement patterns (fuel, clothing,
-- online payments, taxes/fees, Bizum/transfer credit-debit lines) that real
-- statements surfaced as uncategorized despite matching no default rule.
...
-- like 0012, this migration BACKFILLS every existing user by re-running the
-- seed: ON CONFLICT (user_id, name_key) DO NOTHING keeps rows the user
...
-- exist, so only the new patterns above are inserted for existing users.
