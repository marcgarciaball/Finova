-- ---------------------------------------------------------------------------
-- Hand-written: add a 'bizum_income' default income category. Bizum/bank
-- transfers received "from a person" (splitting a bill, being paid back) are
-- money coming back for something already paid — not earnings — so they no
-- longer belong under 'other_income', which the dashboard's "how much do I
-- earn" card sums as real income.
-- Mirrors packages/domain/src/categories/defaults.ts and rules/defaults.ts —
-- keep the three in sync. Like 0010/0012/0023/0024, this migration BACKFILLS
-- every existing user (new category row + re-seeded rules) and additionally
-- moves already-categorized Bizum-received transactions off 'other_income'.
-- ---------------------------------------------------------------------------
...
-- Backfill: give existing users the new category too (the seed function only
-- fires for new signups via the auth trigger).
INSERT INTO public.categories (user_id, name, name_key, kind, is_default, icon_name, color)
SELECT DISTINCT user_id, 'Bizum', 'bizum_income', 'income', true, 'ArrowLeftRight', 'var(--cat-teal)'
FROM public.categories
ON CONFLICT (user_id, name_key) WHERE name_key IS NOT NULL DO NOTHING;--> statement-breakpoint

-- Re-point the two Bizum-received default rules at 'bizum_income' instead
-- of 'other_income' (full re-seed list, mirrors rules/defaults.ts + 0024).
...
BEGIN
  ...
  FROM (VALUES
    ...
    ('pago_bizum', 'Bizum sent', 'other_expense', 'pago bizum', 367),
    ('transferencia_a', 'Transfer sent', 'other_expense', 'transferencia a ', 368),
    ('abono_transferencia', 'Transfer received', 'other_income', 'abono transferencia', 369),
    ('adeudo_recibo', 'Direct debit (adeudo recibo)', 'other_expense', 'adeudo recibo', 370),
...
-- (ON CONFLICT DO NOTHING above only inserts missing ones) — repoint the two
-- Bizum-received rules explicitly for users who already have them.
UPDATE public.categorization_rules r
SET category_id = new_cat.id
FROM public.categories new_cat
WHERE r.name_key IN ('abono_bizum', 'bizum_recibido')
  AND r.is_default = true
  AND new_cat.user_id = r.user_id
  AND new_cat.name_key = 'bizum_income';--> statement-breakpoint

-- Backfill: move already-categorized Bizum-received transactions off
-- 'other_income' into 'bizum_income' — same reasoning as above, applied to
-- existing data (rule/category changes never retroactively touch old rows).
...
WHERE t.category_id = old_cat.id
  ...
  AND new_cat.name_key = 'bizum_income'
  AND (
    lower(t.description) LIKE '%abono bizum%'
    OR lower(t.description) LIKE '%bizum recibido%'
  );
