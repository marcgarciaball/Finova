-- ---------------------------------------------------------------------------
-- Hand-written: add a generic "bar" default rule (restaurants) — a user's
-- card-purchase description ("BAR EL GAT-BENICHEMBLA") matched no existing
-- default rule. Trailing space on the token avoids matching "bar" as a
-- mid-word substring (e.g. "Barcelona", "embarque").
-- Mirrors packages/domain/src/rules/defaults.ts — keep the two in sync.
-- `seed_default_rules()` is CREATE OR REPLACE'd with the fuller list, and,
-- like 0012/0023, this migration BACKFILLS every existing user by re-running
-- the seed: ON CONFLICT (user_id, name_key) DO NOTHING keeps rows the user
-- already has (including edited ones).
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  FROM (VALUES
    ...
    ('abono_bizum', 'Bizum received', 'other_income', 'abono bizum', 366),
...
-- Backfill: re-seed every user that already has categories. Idempotent — the
-- seed's ON CONFLICT (user_id, name_key) DO NOTHING skips rules that already
-- exist, so only the new "bar" pattern above is inserted for existing users.
